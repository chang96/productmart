...
<div style="border: 1px solid #000000; color: #000000" class="col-lg-12 col-xs-12">
    ...
    data.forEach((dat, i)=> {
        const data= dat.data()
        let htm =  `
        <div onclick="goto(this.id)" id=${dat.id} style="margin-top: 10px; color: #000000;  border-radius: 25px; border: 1px solid green; background-color: rgba(0, 122, 57, 0.14)" class="col-lg-12 col-xs-12">
        <div style="" class="row">
            <div class="col-lg-3 col-xs-3">
...
$(document).ready(()=>{
    auth.onAuthStateChanged(async user => {
        if(1 == 1){
            db.collection('requestedProducts').where("owner", "==", user.email).get().then(props=>{
...
const goto = function(id){
    localStorage.setItem("quotedproductid", id)
    window.location = '/dashboard/squote1.html'
}
